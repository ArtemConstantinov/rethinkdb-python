...
from rethinkdb.core import q_printer
from rethinkdb.core.net.repl import Repl
from rethinkdb.core.utilities import EnhancedTuple
...
class ReqlQuery:  # pylint: disable=too-many-public-methods
    ...
    @abstractmethod
    def compose(self, *args, **kwargs) -> Iterable[str]:
        """Compose the Reql query for preaty print"""
        raise NotImplementedError()

    # TODO: add return value
    def run(self, connection: "Connection" | None = None, **kwargs: Unpack[GlobalOptions]):
        """
        ...
        if conn is None:
            ...
            raise errs.ReqlDriverError("ReqlQuery.run must be given a connection to run on.")

        return conn.start(term=self, **kwargs)

    def __str__(self) -> str:
        """
        Return the string representation of the query.
        """
        return q_printer.QueryPrinter(self).query
...
class ReqlConstant(ReqlQuery):
    """
    Rethinkdb constant.
    Maps a real world constant to a representation for the db.
    """

    def __init__(self, statement: str, term_type: int) -> None:
        super().__init__()
...
class ReqlBoolOperQuery(ReqlQuery):
    ...
    def compose(self, *args, **kwargs) -> Iterable[str]:  # pylint: disable=unused-argument
        term_args = [
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        for arg in args:
            if hasattr(arg, "infix"):
                raise errs.ReqlDriverCompileError(
                    f"""
                    Calling '{self.statement}' on result of infix bitwise operator:
                    {q_printer.QueryPrinter(self).query}\n
...
class ReqlTopLevelQuery(ReqlQuery):
    def compose(self, *args, **kwargs) -> Iterable[str]:
        args_ = [*args, *(EnhancedTuple(key, "=", value) for key, value in kwargs.items())]
        return EnhancedTuple(
            "r.", self.statement, "(", EnhancedTuple(*(args_), int_separator=", "), ")"
        )
...
class ReqlMethodQuery(ReqlQuery):
    def compose(self, *args, **kwargs) -> Iterable[str]:
        ...
        if needs_wrap(self._args[0]):
            args_[0] = EnhancedTuple("r.expr(", args_[0], ")")
    ...
    def compose(self, *args, **kwargs) -> Iterable[str]:
        if self.bracket_operator:
            ...
            if needs_wrap(self._args[0]):
                args_[0] = EnhancedTuple("r.expr(", args_[0], ")")
            return EnhancedTuple(
                args_[0], "[", EnhancedTuple(*args_[1:], int_separator=[","]), "]"
            )
...
class ReqlTzinfo(datetime.tzinfo):
    ...
    def __init__(self, offsetstr) -> None:
        super().__init__()

        hours, minutes = (int(val) for val in offsetstr.split(":"))
    ...
    def __init__(self, obj_dict: dict, *args, **kwargs):
        ...
        for key, value in obj_dict.items():
            if not isinstance(key, str):
                raise errs.ReqlDriverCompileError("Object keys must be strings.")
...
class Slice(ReqlBracketQuery):
    ...
    def compose(self, *args, **kwargs) -> Iterable[str]:
        ...
        if self.bracket_operator:
            ...
            return EnhancedTuple(args_[0], "[", args_[1], ":", args_[2], "]")
...
class FunCall(ReqlQuery):
    ...
    def __init__(self, *args, **kwargs) -> None:
        if len(args) == 0:
            raise errs.ReqlDriverCompileError("Expected 1 or more arguments but found 0.")

        args = tuple(func_wrap(args[-1]), *args[:-1])
        super().__init__(*args, **kwargs)
    ...
    def compose(self, *args, **kwargs) -> Iterable[str]:  # pylint: disable=unused-argument
        ...
        if isinstance(self._args[1], Datum):
            args_[1] = EnhancedTuple("r.expr(", args_[1], ")")

        return EnhancedTuple(args_[1], ".do(", args_[0], ")")
...
class Table(ReqlQuery):  # pylint: disable=too-many-public-methods
    ...
    def index_create(self, *args, **kwargs):
        if len(args) > 1:
            args = tuple(args[0], *(func_wrap(arg) for arg in args[1:]))
    ...
    def compose(self, *args, **kwargs) -> Iterable[str]:
        args_ = [*args, *(EnhancedTuple(k, "=", v) for k, v in kwargs.items())]

        if isinstance(self._args[0], DB):
            return EnhancedTuple(
                args_[0], ".table(", EnhancedTuple(*(args_[1:]), int_separator=", "), ")"
            )

        return EnhancedTuple(
            "r.table(", EnhancedTuple(*(args_), int_separator=", "), ")"
        )
...
class ReqlBinary(bytes):
    ...
    def __repr__(self):
        ...
        return f"<binary, {len(self)} byte{plural}{excerpt}>"
...
def _ivar_scan(query: ReqlQuery) -> bool:
    """
...
def needs_wrap(arg: ReqlQuery) -> bool:
    """
...
def expr(
    ...
    if not isinstance(nesting_depth, int):
        raise errs.ReqlDriverCompileError("Second argument to `r.expr` must be a number.")

    if nesting_depth <= 0:
        raise errs.ReqlDriverCompileError("Nesting depth limit exceeded.")
...
AT = TUnion[ReqlQuery, ImplicitVar, list, dict, str, bytes]
RT = TUnion[Func, ReqlQuery, Datum, Binary, MakeObj, MakeArray, ISO8601]

def func_wrap(val: AT) -> RT:
    val = expr(val)
    if _ivar_scan(val):
